    ...
    while (1)
    {
        ...
        if (recvLen < 0)
        {
            if (errno == EAGAIN)
            {
                ...
                {
                    logOut << time_now() << "[ERROR] WRQ ACK time out" << endl;
                    return;
                }
        ...
        while (1)
        {
            ...
            if (recvLen < 0)
            {
                if (errno == EAGAIN)
                {
                    ...
                    if (resendTimes > MAX_RESEND_TIMES)
                    {
                        logOut << time_now() << "[ERROR] Data #" << blockNum << " time out" << endl;
                        return;
                    }
                    logOut << time_now() << "[INFO] Resend " << resendTimes << " times." << endl;
                    sendLen = sendto(sock, sendBuf, readLen + 4, 0, (struct sockaddr *)&serverAddr, sizeof(serverAddr));
                    if (sendLen < 0)
                    {
                        logOut << time_now() << "[ERROR] Cannot send Data #" << blockNum << ". Resent " << resendTimes << "times." << endl;
                        return;
                    }
                    continue;
                }
